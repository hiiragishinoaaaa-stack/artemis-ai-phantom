...
4. スコアから通知ティアを判定する(`scoring.tier_for_score()`)。
   - **HIGH**(`HIGH_SCORE_THRESHOLD`以上、既定90): 🚨 Discord通知
   - **WATCH**(`WATCH_SCORE_THRESHOLD`以上、既定80): ⚠ Discord通知
   - **LOW**(`LOW_SCORE_THRESHOLD`以上、既定70): ログ保存のみ(Discordへは送らない)
...
| `HIGH_SCORE_THRESHOLD` | `90` | これ以上のスコアでHIGH通知 |
| `WATCH_SCORE_THRESHOLD` | `80` | これ以上のスコアでWATCH通知 |
| `LOW_SCORE_THRESHOLD` | `70` | これ以上のスコアでログ保存(Discordへは送らない) |
