...
### 銘柄ごとのTP/SL/グリッド分割数(2026-07)

当初はTP/SL/グリッド分割数を全銘柄(`PERP_SYMBOLS`)共通の1セット
(`PERP_GRID_TAKE_PROFIT_PCT`等)で運用していたが、
`perp_grid_backtest_sweep.py`でBTCUSDT/ETHUSDT/SOLUSDTを実際に比較した
ところ、最適な組み合わせが銘柄によってまったく違うことがわかった:

- **BTCUSDT**: 当時のSL-0.1%は値動きのノイズで刈られやすく、SLを-0.5%
  まで広げた方が勝率・損益分岐勝率との差(マージン)ともに改善した。
- **ETHUSDT/SOLUSDT**: 逆にSL-0.1%のままの方が良く、グリッド分割数を
  300から50〜100程度に減らした方がマージンも最大ドローダウンも改善した。

そのため`config.py`に`PERP_GRID_SYMBOL_DEFAULTS`(実測に基づく銘柄別
既定値、単純に合計損益が一番良かった組み合わせではなく最大ドローダウンが
極端に大きいものは避けて選定)を追加し、`perp_sniper.py`の
`_process_grid_symbol`/`_process_grid_symbol_live`はこれを使うように
変更した。優先順位は次の通り(`config.grid_*_for_symbol`参照):

1. 個別の環境変数上書き(`PERP_GRID_COUNT_<SYMBOL>`/
   `PERP_GRID_TAKE_PROFIT_PCT_<SYMBOL>`/`PERP_GRID_STOP_LOSS_PCT_<SYMBOL>`/
   `PERP_GRID_RANGE_PCT_<SYMBOL>`、`.env.example`参照)
2. `PERP_GRID_SYMBOL_DEFAULTS`の銘柄別既定値(BTCUSDT/ETHUSDT/SOLUSDTの
   みコード側にすでに設定済み)
3. グローバル既定値(`PERP_GRID_TAKE_PROFIT_PCT`等)

⚠️ この検証は15分足1500本(2週間強)という短い期間・BTCUSDT/ETHUSDT/
SOLUSDTの1回の実行結果に基づく(過去にトレンド戦略の検証で「短期間の
好成績はその期間固有の相場環境に助けられていただけ」というオチが
あったのと同じ注意点が当てはまる)。定期的に`perp_grid_backtest_sweep.py`
を再実行して傾向が変わっていないか確認すること。
