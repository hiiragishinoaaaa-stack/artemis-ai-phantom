...
4. スコアから通知ティアを判定する(`scoring.tier_for_score()`)。
   ...
   (コピペ・タップだけで済むことを想定。出来高・注意書き等の詳細はログ
   側にのみ残す)。本文の下に「詳細」「Phantomで開く」のリンクボタンが
   付く(URLを開くだけのボタンなのでDiscord Bot不要、Webhookのみで送信
   できる)。「詳細」ボタンは`DASHBOARD_PUBLIC_URL`(`.env`、任意)が
   設定されている場合のみ、ダッシュボードのその銘柄の詳細ページ
   (`/token/{mint}`、出来高・ユニーク買い手数・上位保有者集中度・通知後の
   時価総額推移などを表示)へ遷移する。「Phantomで開く」は常に付き、
   ...
   [詳細] [Phantomで開く]
   ```
   スコア行の先頭の🚨/⚠は通知ティア(HIGH/WATCH)を表す絵文字で、
   `.env`の`DISCORD_HIGH_TIER_EMOJI`/`DISCORD_WATCH_TIER_EMOJI`で
   Discordサーバーのカスタム絵文字(`<:名前:ID>`形式)に差し替え可能。
   ...
   `.env`の`DISCORD_*_EMOJI`でカスタム絵文字に差し替え可能)。
...
| `DISCORD_HIGH_TIER_EMOJI` | `🚨` | HIGH通知の絵文字(カスタム絵文字`<:名前:ID>`に差し替え可能) |
| `DISCORD_WATCH_TIER_EMOJI` | `⚠` | WATCH通知の絵文字(同上) |
