...
4. スコアから通知ティアを判定する(`scoring.tier_for_score()`)。
   ...
   ばかりで、よほど注目度が高い場合を除き★は付きにくい。そこで、既に
   通知済みのトークンが**後のチェックポイントで実際にユニーク買い手★3つに
   到達した瞬間**、通常とは別の`DISCORD_FOLLOWUP_WEBHOOK_URL`(`.env`、任意)
   へ「🔥 ユニーク買い手が★3つに到達」という追い通知を1トークンにつき
...
| `DISCORD_FOLLOWUP_WEBHOOK_URL` | (空) | 通知済みトークンが後からユニーク買い手★3つに到達した時だけ送る追い通知のWebhook URL(任意) |
