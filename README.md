...
4. スコアから通知ティアを判定する(`scoring.tier_for_score()`)。
   ...
   「詳細」ボタンの遷移先ページとログ側にのみ残す)。本文の下に「詳細」
   「Phantomで開く」のリンクボタンが付く(URLを開くだけのボタンなので
   Discord Bot不要、Webhookのみで送信できる)。「詳細」ボタンは
   `DASHBOARD_PUBLIC_URL`(`.env`、任意)が設定されている場合のみ、
   ダッシュボードのその銘柄の詳細ページ(`/token/{mint}`、mintアドレス・
   出来高・ユニーク買い手数・上位保有者集中度・通知後の時価総額推移などを
   表示)へ遷移する。「Phantomで開く」は常に付き、`PHANTOM_REFERRAL_ID`
   (`.env`、任意)が設定されていれば紹介コードを付与する。例:
   ```
   ⭐⭐ 80/100 ✅
   Some Coin ($SOME) 🐦✈️
   [詳細] [Phantomで開く]
