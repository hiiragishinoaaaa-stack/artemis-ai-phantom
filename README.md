...
**実際の取引所への発注は未実装。** `PERP_PAPER_TRADING_ENABLED=true`
(既定ON、実資金を動かさないため安全側の既定にしている)にすると、
「もし建てていたら」のペーパートレード(モック)としてレバレッジ込みの
損益をシミュレートし、その結果もDiscordへ通知する(`perp_paper_trader.py`)。
本物の取引所APIでの自動発注に拡張したくなった場合は、`perp_paper_trader.
PaperPerpTracker`と同じインターフェース(`open_position`/`close_position`)
を持つ実発注版クラスに差し替える設計を想定している(そこは今回未実装。
レバレッジのかかったパーペチュアルの自動発注は現物のミームコイン以上に
リスクが高いため、実装する場合は改めて相談してから着手すること)。
