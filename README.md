...
4. スコアから通知ティアを判定する(`scoring.tier_for_score()`)。
   ...
   スコアが**100点満点、かつユニーク買い手★3つ**の場合のみ、上記の
   通常チャンネルに加えて`DISCORD_PERFECT_SCORE_WEBHOOK_URL`(`.env`、
   任意)で指定した別のWebhookにも同じ内容を送る。100点でも★3つに
   満たない(少数のウォレットだけで押し上げた)場合は通常チャンネルの
   みに送る。本当に選び抜かれたものだけを集めた専用チャンネルを別に
...
| `DISCORD_PERFECT_SCORE_WEBHOOK_URL` | (空) | スコア100点満点かつ★3つのものだけを追加通知する専用チャンネルのWebhook URL(任意) |
