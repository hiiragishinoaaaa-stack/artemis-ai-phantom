...
class OutcomeTracker:
    ...
    def register(
        ...
        self._outcomes[mint] = TrackedOutcome(
            ...
            market_cap_at_notify=market_cap_sol,
            last_market_cap_sol=market_cap_sol,
        )

    def update_market_cap(self, mint: str, market_cap_sol: float) -> None:
        """追跡中トークンの売買イベントを受けて最新の時価総額を更新する。"""
        outcome = self._outcomes.get(mint)
        if outcome is not None:
            outcome.last_market_cap_sol = market_cap_sol
    ...
    def record_and_advance(self, outcome: TrackedOutcome) -> None:
        ...
        checkpoint_seconds = config.OUTCOME_CHECKPOINTS_SECONDS[outcome.checkpoint_index]
        if outcome.market_cap_at_notify > 0:
            change_pct = (
                (outcome.last_market_cap_sol - outcome.market_cap_at_notify)
                / outcome.market_cap_at_notify
                * 100
            )
        else:
            change_pct = 0.0
        ...
        record = {
            ...
            "market_cap_at_notify_sol": outcome.market_cap_at_notify,
            "market_cap_now_sol": outcome.last_market_cap_sol,
